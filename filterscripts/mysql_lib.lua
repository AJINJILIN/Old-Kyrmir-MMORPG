...
-- Handler
local handler = mysql_connect("localhost", "root", "000000", "mmo");
...
function reconnectMySQL()
	...
	mysql_close(handler);
	handler = mysql_connect("localhost", "root", "000000", "mmo");
end
				...
				if checkMySQLConnection() == true then
					...
					local adml = Player[playerid].adm
							if Player[playerid].adm[1] == "5" then
								...
								Player[playerid].adm = 5;
								SendPlayerMessage(playerid,0,255,255,string.format("%s %s %s %s","Права доступа: Администратор","[id: "adml,"]"));
							elseif 
								Player[playerid].adm[1] == "4" then
								table.remove(Player[playerid].adm , 1);
								Player[playerid].adm = 4;
								SendPlayerMessage(playerid,0,255,255,string.format("%s %s %s %s","Права доступа: Гл. Модератор","[id: "adml,"]"));
							elseif 
								Player[playerid].adm[1] == "8" then
								table.remove(Player[playerid].adm , 1);
								Player[playerid].adm = 8;
								SendPlayerMessage(playerid,0,255,255,string.format("%s %s %s %s","Права доступа: Модератор","[id: "adml,"]"));
							else
								table.remove(Player[playerid].adm , 1);
								Player[playerid].adm = 0;
							end
							if res then
								mysql_free_result(res);
							end
...
function MySQL_Save(playerid)
	...
	if Player[playerid].LoggedIn == true then
		...
		if checkMySQLConnection() == true then
			...
			local world = "OLDWORLD.ZEN" --na sztywno i chooj
