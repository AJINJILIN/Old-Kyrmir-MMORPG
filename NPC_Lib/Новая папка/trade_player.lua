...
function StartPlayersTade(playerid, focus_id, params)
	...
	if result == 1 then -- jeїeli wszystkie parametry s№ wypeіnione
						...
						if Player[focus_id].Trade == false then -- jeїeli osoba na ktуr№ patrzysz nie handluje
							if Player[playerid].Trade == false then -- jeїeli Ty nie handlujesz
								if Player[playerid].Dialog == false then  -- jeїeli nie gadasz z їadnym npc(?)
									...
									SendPlayerMessage(playerid, 255, 0, 0, "У Вас открыт диалог с NPC и Вы хотите торговать ;E")
		...
		SendPlayerMessage(playerid, 255, 0, 0, "Пиши: /продать (слот) (кол-во) (цена), чтобы торговать.");
	end
end
...
function TradeAccept(focid)
	if Player[focid].Trade == true then
		...
		if GetPlayerGold(focid) >= Player[Player[focid].Focus].Gold then -- To drugie od focusa bo akceptuje ten komu zostaіo zaoferowane ;E
			...
			SendPlayerMessage(focid, 0, 255, 0, "Сделка успешно произведена.");
			SendPlayerMessage(Player[focid].Focus, 0, 255, 0, "Сделка успешно произведена.");
			...
			LogString("Logs_Trade", GetPlayerName(focid).." buyed "..GetPlayerName(Player[focid].Focus).." "..GetItemName(Player[Player[focid].Focus].Item).." amount: "..Player[Player[focid].Focus].Amount.." for: "..Player[Player[focid].Focus].Gold.." gold");
			...
			if equipped == 1 then
				...
				if string.upper(item_instance) == "ITMI_GOLD" then
					if Player[playerid].Amount <= GetPlayerGold(playerid) then
						...
						SendPlayerMessage(Player[playerid].Focus, 0, 255, 0, "Пиши /согл, чтобы подтвердить сделку.");
						
						LogString("Logs_GoldenTrade_old", GetPlayerName(playerid).." предложил "..GetPlayerName(Player[playerid].Focus).." "..Player[playerid].Amount.." золота".." за "..Player[playerid].Gold.." золота.");
					else
					...
					if Player[playerid].Amount <= amount then
						if not(Player[playerid].Amount <= GetItemAmountPlayer(playerid, sprawdz)) then
							...
							Ban(playerid, 1);
							SendMessageToAll(255, 0, 0, GetPlayerName(playerid).." пытался сжульничать, с помощью торговли и багов, как он считал.");
							SendMessageToAll(255, 0, 0, "Но все предусмотрено и теперь он в бане, благодаря античиту!");
