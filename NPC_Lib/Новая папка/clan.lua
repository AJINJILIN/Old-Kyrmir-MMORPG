...
function OnPlayerCommandText(playerid, cmdtext)
	...
	if cmdtext == "/создать" or cmdtext == "/create" then
	CMD_CreateClan(leaderid, params);
	elseif cmdtext == "/квраг" or cmdtext == "/cenemy" then
	AddEnemy(playerid, tag, enemy_tag);	
	elseif cmdtext == "/кудалить" or cmdtext == "/cremove" then
	RemoveEnemy(playerid, tag, enemy_tag);
	elseif cmdtext == "/добучаст" or cmdtext == "/addmember" then
	AddMember(playerid, memberid, tag);
	elseif cmdtext == "/удучаст" or cmdtext == "/rmember" then
	RemoveMember(playerid, id, tag);
	elseif cmdtext == "/тест" or "/test" then	
	Clan_OnPlayerConnect(playerid);
	end  
end
...
function CreateClan(leaderid, name, tag)
	...
	if CheckExistClan(name, tag) == false then
		...
		SaveClan(tag);
		CreateClanDraws( tag );
		SendPlayerMessage(leaderid, 0, 255, 0, "Клас успешно создан.");
