...
local cmd, params = GetCommand(cmdtext);
	...
	if cmd == "/создать" or cmd == "/create" then
	CMD_CreateClan(playerid, params);
	end
	
	if cmd == "/квраг" or cmd == "/cenemy" then
	AddEnemy(playerid, tag, enemy_tag);	
	end
	if cmd == "/кудалить" or cmd == "/cremove" then
	RemoveEnemy(playerid, tag, enemy_tag);
	end
	if cmd == "/добучаст" or cmd == "/addmember" then
	AddMember(playerid, memberid, tag);
	end
	if cmd == "/удучаст" or cmd == "/rmember" then
	RemoveMember(playerid, id, tag);
	end
	if cmd == "/тест" or cmd ==  "/test" then	
	Clan_OnPlayerConnect(playerid);
	end
	if cmd == "/лидер" or cmd ==  "/leader" then	
	SendPlayerMessage(playerid, 255, 0, 0, "test");
	CheckLeader(playerid);
	end  
end

function CheckLeader(playerid)
if GetPlayerLeaderClan(playerid) == false then
print ("false");
SendPlayerMessage(playerid, 255, 0, 0, "false");
end
if GetPlayerLeaderClan(playerid) == true then
print ("true");
SendPlayerMessage(playerid, 255, 0, 0, "true");
end
if GetPlayerLeaderClan(playerid) == nil then
print ("error");
SendPlayerMessage(playerid, 255, 0, 0, "nil");
end
end

function CMD_CreateClan(playerid, params)
		...
		if result == 1 then
			...
			if string.len(name) <= ClanName then
				...
				if string.len(tag) <= ClanTag then
					...
					CreateClan(playerid, string.gsub(name, "_", " "), tag);
					
				else
				
					SendPlayerMessage(playerid, 255, 0, 0, "Клан-таг не может содержать больше 3-х знаков!");
				
				end
				
			else
				
				SendPlayerMessage(playerid, 255, 0, 0, "Название клана не может содержать больше 25-ти знаков!");
			
			end
			
		else
		
			SendPlayerMessage(playerid, 255, 0, 0, "Пиши /создать (название) (клан-таг)");
		
		end
	
	else
	
		SendPlayerMessage(playerid, 255, 255, 0, "Вы уже лидер клана!");
...
function AddMember(playerid, memberid, tag)
	...
	if GetPlayerLeaderClan(playerid) == true then
		...
		if Player[memberid].Clan ~= nil then
...
function GetPlayerLeaderClan(playerid)
	...
	if Player[playerid].Clan ~= nil then
		...
		if Clan[Player[playerid].Clan].Leader == HideTag(playerid) then
...
function HideTag(playerid)
	...
	if Player[playerid].Clan ~= nil then
		...
		local nick = string.gsub(GetPlayerName(playerid), "|"..Player[playerid].Clan.."|", "");
