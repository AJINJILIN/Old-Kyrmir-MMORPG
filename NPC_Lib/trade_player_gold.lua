...
function GoldTrade(playerid, focusid, params)
	...
	if result == 1 then
								...
								if ile > 0 then
									if zaile > -1 then
										if GetPlayerGold(playerid) >= ile then
											...
											SendPlayerMessage(focusid, 0, 255, 0, "Используй команду /гсогл, чтобы подтвердить торговлю.");
		...
		SendPlayerMessage(playerid, 255, 0, 0, "Используй команду /гпродать (Ваше золото) (за сколько золота), чтобы торговать золотом.");
	end
end
