...
<meta name="description" content="Online Players" />
<meta name="keywords" content="online,lista graczy online,onlajn kyrmir"/>
<meta name="author" content="midas"/>

<title>Kyrmir MMORPG - Online List</title>
<style type="text/css">
body{
...
<?php
	...
	$connection = null;
		...
		$db = array( 'localhost', 'ot', 'Ei', 'KyrBase' );
        connect($db[0], $db[1], $db[2], $db[3]);
           
        $objects_array = array();
		$result = mysql_query("SELECT `lvl`, `nick` FROM player WHERE `online` = '1' ORDER BY `lvl` DESC");
		...
		global $ig;
		echo '<font color="gold">Ilość Graczy Online: <b>'.$ig.'</b></font>';
        ...
        if($aktualna_strona > 1){
            echo '<font color="white"><a href="online.php?strona='. ($aktualna_strona - 1) .'"><</a></font>';
        }
        echo ' ';
